  ...
  return (
    <div className="min-h-screen bg-slate-50 pb-28">
      ...
      >
        ...
        <Card variant="default" padding="none" className="overflow-hidden border-2 border-slate-300 relative bg-slate-950 rounded-3xl">
          <div className="relative aspect-video w-full overflow-hidden">
            ...
            {isScanning ? (
              <div className="absolute inset-0 bg-slate-950/45 flex flex-col items-center justify-center p-4">
                <div className="bg-slate-950/85 backdrop-blur-sm px-4 py-2.5 rounded-2xl border border-emerald-500/60 flex items-center gap-2.5 text-white shadow-lg">
              ...
              <div className="absolute inset-5 border-2 border-emerald-400 rounded-2xl pointer-events-none flex flex-col justify-between p-2.5">
                <div className="flex justify-between items-center">
                  <span className="bg-emerald-600 text-white text-xs font-black px-2.5 py-1 rounded-lg shadow-xs">
                    PCB • 94% Match
                  </span>
                  <span className="w-3 h-3 bg-emerald-400 rounded-full" />
                </div>
                <div className="self-end bg-slate-950/80 text-white text-xs font-mono font-bold px-2.5 py-1 rounded-md">
                  FR-4 / Cu Trace
                </div>
              </div>
            )}
        ...
        {isScanning ? (
          ...
          result && (
            <div className="space-y-4">
              ...
              <Card variant="highlight" className="p-5 border-2 border-emerald-500 bg-emerald-50/70 shadow-xs">
                <div className="flex items-start justify-between gap-3">
                  <div>
                    ...
                    <h2 className="text-2xl sm:text-3xl font-black text-slate-950 mt-0.5 flex items-center gap-2">
                      <Cpu className="w-7 h-7 sm:w-8 sm:h-8 text-emerald-700 stroke-[2.2]" />
                      <span>{language === 'hi' ? result.materialNameHi : result.materialName}</span>
                    </h2>
                  </div>

                  <div className="text-right">
                    <Badge variant="emerald" size="lg" icon={<CheckCircle2 className="w-4 h-4 text-emerald-700" />}>
                      {t('confidence', { confidence: result.confidence })}
                    </Badge>
                  </div>
                </div>

                <div className="mt-4 pt-3 border-t border-emerald-200/80 grid grid-cols-2 gap-3 text-left">
                  <div>
